...
class Att(nn.Module):
    def __init__(self, d_model = 64, input_dim=1, num_heads=1, max_len=5000):
        super(Att, self).__init__()
        self.embedding = nn.Linear(input_dim, d_model)  # Learnable Embedding (1차원 -> d_model)
        ...
        self.attention_x3 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        # self.attention_x4 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        

        self.attention_2 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        self.attention_3 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        self.attention_4 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        self.attention_5 = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads)
        self.lstm = nn.LSTM(d_model, 64, 9, batch_first=True)
    ...
    def forward(self, x):
        h0 = torch.zeros(9, x.size(0), 64).to(x.device)
        c0 = torch.zeros(9, x.size(0), 64).to(x.device)
        # print(x.shape)
        #torch.Size([1, 9, 1])

        # x: [batch_size, time_steps, 1] 형태로 들어옴
        ...
        x = x.transpose(0, 1)  # (9, 1, 1) (sequence_length, batch_size, embed_dim)

        x_output, _ = self.attention_x(x, x, x)
        x_output, _ = self.attention_x1(x_output, x_output, x_output)
        x_output, _ = self.attention_x2(x_output, x_output, x_output)
        x_output, _ = self.attention_x3(x_output, x_output, x_output)
        # x_output, _ = self.attention_x4(x_output, x_output, x_output)

        x_output_1, _ = self.attention_2(x_output, x_output, x_output)
        x_output_2, _ = self.attention_3(x_output, x_output, x_output)
        x_output, _ = self.attention_4(x_output_2, x_output_1, x_output_1)
        x_output, _ = self.attention_5(x_output, x_output, x_output)
        x_output = x_output.transpose(0,1)

        out, _ = self.lstm(x_output, (h0, c0))
        x = self.fc(out[:, -1, :])
        x = self.fc2(x)
        # x = self.fc3(x)
...
# 결과 저장
sample_submission.to_csv('./baseline_v6.csv', index=False, encoding='utf-8')
