...
def process_data(raw_file, 산지공판장_file, 전국도매_file, 품목명, scaler=None):
    ...
    if conditions[품목명]['공판장']:
        ...
        if 품목명 != '건고추':
            filtered_공판장 = filtered_공판장.rename(columns={'공판장_평균가(원/kg)': '공판장_평균가격'})
            filtered_공판장['공판장_평균가격'] = filtered_공판장['공판장_평균가격'] * 단위.get(품목명) 
    ...
    if conditions[품목명]['도매']:
        ...
        if 품목명 != '건고추':
            filtered_도매 = filtered_도매.rename(columns={'도매_평균가(원/kg)': '도매_평균가격'})
            filtered_도매['도매_평균가격'] = filtered_도매['도매_평균가격'] * 단위.get(품목명) 
        filtered_data = filtered_data.merge(filtered_도매, on='시점', how='left')

    

    # 수치형 컬럼 처리
    numeric_columns = ['평균가격(원)'] if 품목명 == '건고추' else ['평균가격(원)', '공판장_평균가격', '도매_평균가격']
...
class Att(nn.Module):
    ...
    def forward(self, x):
        ...
        x_output, _ = self.attention_3(x_output_2, x_output_1, x_output_1)
...
# 결과 저장
sample_submission.to_csv('./baseline_v5.csv', index=False, encoding='utf-8')
