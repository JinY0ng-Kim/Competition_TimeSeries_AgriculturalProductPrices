...
품목_리스트 = ['배추', '대파','양파', '깐마늘(국산)', '배', '건고추',  '무', '상추', '사과', '감자']
...
def process_data(raw_file, 산지공판장_file, 전국도매_file, 품목명, scaler=None):
    ...
    '감자': {
        ...
        '공판장': None,
        '도매': None #{'시장명': ['*전국도매시장', '강릉', '광주각화', '광주서부','대전오정', 
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        div_term = torch.exp(torch.arange(0, d_model, d_model).float() * (-torch.log(torch.tensor(10000.0)) / d_model))
...
class Att(nn.Module):
    def __init__(self, input_dim, d_model = 720, num_heads=4, max_len=5000):
        ...
        self.fc = nn.Linear(d_model*input_dim, d_model*input_dim)
        self.fc2 = nn.Linear(d_model*input_dim, 3)
...
for 품목명 in pbar_outer:
    ...
    #     model = Att().to(device)
    # else:
    #     model = Att(d_model=input_dims[품목명]).to(device)
    model = Att(input_dim=input_dims[품목명]).to(device)
    criterion = nn.L1Loss()
...
# 결과 저장
sample_submission.to_csv('./새로운시작_v1.csv', index=False, encoding='utf-8')
