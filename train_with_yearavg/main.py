...
def process_data(raw_file, 산지공판장_file, 전국도매_file, 품목명, scaler=None, scaler2=None):
    ...
    # 수치형 컬럼 처리
    backbone_data = filtered_data[['시점'] + list(['평년 평균가격(원)'])]
    backbone_data[['평년 평균가격(원)']] = backbone_data[['평년 평균가격(원)']].fillna(0)

    numeric_columns = ['평균가격(원)']
    main_data = filtered_data[['시점'] + list(numeric_columns)]
    main_data[numeric_columns] = main_data[numeric_columns].fillna(0)
    ...
    if scaler is None:
        ...
        main_data[numeric_columns] = scaler.fit_transform(main_data[numeric_columns])
        backbone_data[['평년 평균가격(원)']] = scaler2.fit_transform(backbone_data[['평년 평균가격(원)']])
    else:
        main_data[numeric_columns] = scaler.transform(main_data[numeric_columns])
        backbone_data[['평년 평균가격(원)']] = scaler2.transform(backbone_data[['평년 평균가격(원)']])
        

    # print(filtered_data)
    return main_data, backbone_data, scaler, scaler2
...
class Att(nn.Module):
    def __init__(self, d_model = 180, num_heads=9, max_len=5000):
        ...
        self.cross = nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads, dropout=0.001)
        self.fc = nn.Linear(180, 180)
        self.fc2 = nn.Linear(180, 3)

    def load_pretrained_weights(self):
        pretrained_dict = torch.load(f'/home/work/PFVBG/yong/a/skrrrr/backbone_models/best_model_{품목명}.pth')
        self.backbone.load_state_dict(pretrained_dict, strict=False)

    def freeze_backbone(self):
        # Backbone의 모든 파라미터를 고정 (Freeze)
        for param in self.backbone.parameters():
            param.requires_grad = False

    def forward(self, x, y):

        y_backbone = self.backbone(y)
...
def train_model(model, train_loader, bb_train_loader, criterion, optimizer, num_epochs):
    model.train()
    total_loss = 0
    for (batch_x, batch_y), (bb_x, bb_y) in zip(train_loader, bb_train_loader):
        batch_x, batch_y = batch_x.to(device), batch_y.to(device)
        bb_x = bb_x.to(device)
        optimizer.zero_grad()
        outputs = model(batch_x, bb_x)
...
def evaluate_model(model, test_loader, bb_test_loader, criterion):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for (batch_x, batch_y), (bb_x, bb_y) in zip(test_loader, bb_test_loader):
            batch_x, batch_y = batch_x.to(device), batch_y.to(device)
            bb_x = bb_x.to(device)
            outputs = model(batch_x, bb_x)
            loss = criterion(outputs, batch_y)
...
for 품목명 in pbar_outer:
    ...
    train_data, backbone_data, scaler, scaler2 = process_data("/home/work/PFVBG/yong/a/dataset/train/train.csv", 
    ...
    main_dataset = AgriculturePriceDataset(train_data)
    backbone_data = AgriculturePriceDataset(train_data)

    # 데이터를 train과 validation으로 분할
    main_train_data, main_val_data = train_test_split(main_dataset, test_size=0.2, random_state=42)

    bb_train_data, bb_val_data = train_test_split(backbone_data, test_size=0.2, random_state=42)
    
    main_train_loader = DataLoader(main_train_data, CFG.batch_size, shuffle=True)
    main_val_loader = DataLoader(main_val_data, CFG.batch_size, shuffle=False)

    bb_train_loader = DataLoader(bb_train_data, CFG.batch_size, shuffle=True)
    bb_val_loader = DataLoader(bb_val_data, CFG.batch_size, shuffle=False)

    input_size = len(main_dataset.numeric_columns)
    
    for batch_x, batch_y in main_train_loader:
    ...
    summary(model, input_size=[(106, 9, 1), (106, 9, 1)])
    ...
    for epoch in range(CFG.epoch):
        train_loss = train_model(model, main_train_loader, bb_train_loader, criterion, optimizer, CFG.epoch)
        val_loss = evaluate_model(model, main_val_loader, bb_val_loader, criterion)
    ...
    for i in pbar_inner:
        ...
        test_data, bb_test_data, _, _ = process_data(test_file, 산지공판장_file, 전국도매_file, 품목명, scaler=품목별_scalers[품목명], scaler2=scaler2)
        test_dataset = AgriculturePriceDataset(test_data, is_test=True)
        bb_test_dataset = AgriculturePriceDataset(bb_test_data, is_test=True)
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)
        bb_test_loader = DataLoader(bb_test_dataset, batch_size=1, shuffle=False)

        model.eval()
        predictions = []
        with torch.no_grad():
            for (batch), (bb_x) in zip(test_loader, bb_test_loader):
                batch, bb_x = batch.to(device), bb_x.to(device)
                output = model(batch, bb_x)
...
# 결과 저장
sample_submission.to_csv('./baseline_v100.csv', index=False, encoding='utf-8')
